...
const SAVE_PATH := "user://idle_battler_save.json"
...
func calculate_offline_reward(state: Dictionary) -> Dictionary:
	...
	var last_saved := int(state.get("last_saved_unix", now))
	var elapsed := max(0, now - last_saved)
	var reward_rate := float(state.get("coin_rate", 1.0))
	var reward := int(min(elapsed, 3600) * reward_rate)
	return {
		"elapsed": elapsed,
		"coins": reward
	}
